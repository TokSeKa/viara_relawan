...
<body>
            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav ms-auto gap-2 align-items-center">
                        ...
                        {{-- 2. TOMBOL AUDIO MUSIK (Background Music) --}}
                        @if($activeMusic)
                        <li class="nav-item me-2">
                            ...
                            {{-- Element Audio disiapkan tapi dikontrol JS --}}
    ...
    {{-- SCRIPT 2: REAL-TIME NOTIFICATION SYSTEM (NO FILE UPLOAD) --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // INI ADALAH SUARA "TING" DALAM BENTUK TEKS (BASE64)
            // Tidak perlu upload file mp3 lagi.
            ...
            const notifAudio = new Audio(soundBase64);
            ...
            function unlockNotifAudio() {
                if (!audioUnlocked) {
                    // Mainkan suara volume 0 sebentar untuk "memancing" browser
                    notifAudio.volume = 0;
                    notifAudio.play().then(() => {
                        notifAudio.pause();
                        notifAudio.currentTime = 0;
                        notifAudio.volume = 1; // Balikin volumenya
                        audioUnlocked = true;
                        // Hapus listener agar tidak berat
                        document.removeEventListener('click', unlockNotifAudio);
                        document.removeEventListener('keydown', unlockNotifAudio);
                    }).catch(e => {
            ...
            // Pasang pancingan di seluruh dokumen
            document.addEventListener('click', unlockNotifAudio);
                ...
                fetch("{{ route('notifikasi.check') }}", {
                    ...
                    .then(data => {
                        if (data.status === 'found') {
                            ...
                            // A. Bunyikan Suara Base64
                            if (audioUnlocked) {
                                notifAudio.currentTime = 0;
                                notifAudio.play().catch(e => console.error("Gagal bunyi:", e));
                            }

                            // B. Tampilkan Notifikasi Browser
                            if (Notification.permission === "granted") {
                                const notif = new Notification("Komunitas Relawan: " + data.title, {
                                    body: data.message,
