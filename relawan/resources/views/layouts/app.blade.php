...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // [1] TEMPAT ISI BASE64 AUDIO (Format: "data:audio/mp3;base64,......")
            ...
            // Cek apakah base64 sudah diisi
            let notifAudio = null;
            if (soundBase64 && soundBase64.length > 50) {
                notifAudio = new Audio(soundBase64);
                // console.log("🎵 Audio Notifikasi Siap (Base64)");
            } else {
            ...
            function unlockAudio() {
                if (!audioUnlocked && notifAudio) {
                    notifAudio.play().then(() => {
                        ...
                        // Lepas listener agar ringan
                        document.removeEventListener('click', unlockAudio);
                        document.removeEventListener('keydown', unlockAudio);
                    }).catch(e => {
                        // Silent fail: Browser belum mengizinkan
                    });
                }
            }

            // Pasang pancingan unlock
            ...
            setInterval(function() {
                fetch("{{ route('notifikasi.check') }}", {
                    ...
                    .then(res => res.json())
                    .then(data => {
                        // Pastikan data count valid
                        if (typeof data.count !== 'undefined') {

                            // JIKA JUMLAH BERTAMBAH
                            if (data.count > lastCount) {
                                // console.log(`Notif Baru! Total: ${data.count}`);
                                ...
                                } else if (!audioUnlocked && notifAudio) {
                                    console.warn("⚠️ Notif masuk, tapi user belum klik halaman (Audio Locked).");
                                }
                            }
                    ...
                    .catch(err => {
                        // Silent error jika route not found / server mati
                    });
